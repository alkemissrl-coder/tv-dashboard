...
async function fetchCSV(url){
 try{ const res=await fetch(url+'?_ts='+Date.now()); if(!res.ok) throw new Error('HTTP '+res.status);
  ...
  console.error('Errore CSV',url,err);
  showError('Errore lettura '+url+': '+err.message+' — Verifica che i CSV siano nella STESSA cartella di index.html');
  return [];
 }
}

function sliceRange(data, days=null, months=null){
...
function areaTrace(data, fillColor, lineColor){
 return { x:data.map(d=>d.date), y:data.map(d=>d.value), mode:'lines',
  line:{color:lineColor,width:4}, fill:'tozeroy', fillcolor:fillColor, hoverinfo:'x+y' };
}
...
function makeQR(){
 ...
 new QRCode(el, { text: window.DASHBOARD_CONFIG.qrLink, width:px, height:px,
